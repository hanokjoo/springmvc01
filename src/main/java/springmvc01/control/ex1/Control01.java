 ...
 *    @Component를 붙여도 되지만 페이지 컨트롤러의 경우 객체의 역할을 명시하기 위해 @Controller 애노테이션을 사용한다.
 *    객체의 이름은 생략해도 된다.
 * => 클라이언트 요청에 대해 호출될 메서드를 만들고, 메서드 선언부에 @RequestMapping을 붙인다. 그리고 URL을 부여한다.
...
public class Control01 {
  ...
  public String test01() {
    ...
    // 리턴 값은 인클루드/포워드 할 뷰 컴포넌트의 경로이다.
    return "/ex1/test01.jsp";
  }
  
  /* 만약 리턴 값을 지정하지 않으면, 기본으로 해당 URL의 폴더로 가서 뷰 컴포넌트를 찾는다.
   * 단 현재 URL 경로에 요청한 URL 경로가 덧붙여진다.
 ...
 * 3) DispatcherServlet은 자신이 갖고 있는 스프링 IoC 컨테이너에서 "/ex1/test" 요청을 처리할 객체와 그 메서드를 찾는다.
 *    (java89-project-java의 RequestThread.java 참조)
