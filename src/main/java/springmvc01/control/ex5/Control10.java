...
public class Control10 {
  ...
  public ModelAndView test02() {
    // ModelAndView 객체에 뷰 컴퍼넌트 URL을 저장하여 리턴한다.
    ModelAndView mv = new ModelAndView();
    mv.setViewName("ex5/jsp02");
    return mv;
  }
  
  /* 뷰 컴포넌트를 경유하지 안고 바로 콘텐츠를 클라이언트에게 보내기1
   * => 문자열로 콘텐츠를 리턴하라.
   * => 문자열이 콘텐츠임을 표시하기 위해 @ResponseBody 애노테이션을 붙여라.
   */
  ...
  /* 뷰 컴포넌트를 경유하지 안고 바로 콘텐츠를 클라이언트에게 보내기2
   * => ResponseEntity 객체를 리턴한다.
   * => DispatcherServlet(프론트 컨트롤러)은 리턴 값의 타입이 ResponseEntity이면,
   *    JSP를 실행하지 않고 이 객체에 들어있는 컨텐츠와 헤더를 가지고 클라이언트로 바로 출력한다.
   */
  @RequestMapping(path="/ex5/test04")
