...
public class Control07 {
  ...
  * => 메서드의 파라미터로 선언하면 프론트 컨트롤러(DispatcherServlet)가 메서드를 호출하기 전에
  *    그 메서드의 파리미터를 분석하여, 그에 해당하는 값을 넘겨준다.
  */
  ...
  public String test01(
    ...
    /* 주의!
     * HttpServletResponse 객체를 파라미터로 받게 되면, 출력의 책임을 메서드에 떠넘기기 때문에
  ...
  public void test03(
      ...
      @RequestParam("age") int age, /* 프론트 컨트롤러가 문자열을 자바 원시타입으로 변환해준다.*/
  ...
  /* 요청 파라미터 값 필수/선택 지정하기
  * => 만약 파라미터의 타입이 자바 원시 타입이라면,
  *    요청 파라미터 값이 없을 때 예외가 발생하는데
  *    이런 경우에는 required=false 대신 default 속성을 사용하여 기본 값을 지정하라.
  ...
  */
  ...
  /* @ReqeustParam에서 파라미터명 생략
   * => 파라미터명이 변수명과 같다면 생략 가능하다.
   */
